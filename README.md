...
- Firebase Analytics (`firebase/analytics`) is initialized in `webapp/src/auth/firebase.ts` and is **disabled automatically in development mode** (`VITE_MODE === 'development'` → `analytics` is `null`). In staging/production, `getAnalytics(app)` is called and requires the Firebase project to have Analytics enabled with a valid Measurement ID in the project config. All event calls route through the `useAnalytics()` hook (`src/hooks/useAnalytics.ts`) — 7 core events: `app_open`, `login_completed`, `registration_completed`, `store_added`, `store_viewed`, `search_performed`, `tab_switched`. No PII is included in event params.
